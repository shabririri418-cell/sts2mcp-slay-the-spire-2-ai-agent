...
- Before narrating a run in Chinese, read [references/tower-p-language.md](references/tower-p-language.md) and [references/tower-p-expression-bank.md](references/tower-p-expression-bank.md) completely. The language guide controls semantics and density; the corpus-derived bank controls scene selection, variation, continuity, and callbacks. Both are mandatory, not optional flavor.
...
- Keep a private narration state containing the latest boast, named roles, missing or protected object, unresolved lesson, and phrase families used in the last five resolved updates. Use it to pay off or invert earlier lines. Do not repeat the high-frequency anchors `这下看懂了`, `吓哭了`, `老霸道了`, `悔恨了`, `关键起防`, or `气笑了` inside that five-update window unless completing a deliberate callback.
